...
function getAttributes(options){
  ...
  var self = this,
      ...
      names = options.names || 'All',
      query = {};
  if(useCached && self._attributes){
    console.log('using cached', self._attributes);
    return Q.resolve(self._attributes);
  }else{
    // TODO put this business in utils
    if(options.names instanceof Array){
      options.names.forEach(function(name, i){
        query['AttributeName.' + (i + 1)]  = name;
      });
    }else{
      query['AttributeName.1'] = 'All';
    } 
