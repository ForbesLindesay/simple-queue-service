...
function Encoding(options){
  ...
  readOnlyProperty(this, 'decode', function(body){
    if(options.base64){
  ...
  readOnlyProperty(this, 'encode', function(obj){
    if(options.parseJSON || !typeof(obj) === 'string'){
      obj = JSON.stringify(obj);
    }
    
    if(options.base64){
      obj = new Buffer(obj, 'utf-8').toString('base64');
    }
    return obj;
  });
}
 ...
 *  * `region` {string} SQS endpoint region [default: `'us-east'`]
...
function Queue(queueName, options) {
  ...
  readOnlyProperty(this, 'encoding',new Encoding(options));
...
function send(message, options) {
  var query = {};
  query.MessageBody = this.encoding.encode(message);
  options = options || {};
...
/** 
 * Deletes this queue
 * @return A promise that will resolve when the queue is no longer among the living.
 */
function deleteQueue(){
  return call(this, 'DeleteQueue',{});
}

Queue.prototype.delete = deleteQueue;